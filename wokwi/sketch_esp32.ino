// Smart Plant Communicator - ESP32 Wokwi simulation variant.
// Three sensors: soil moisture (analog), water tank level (digital), pump relay.
// Logic identical to plant_communicator.ino. Use this ONLY to test in Wokwi.
...
// ESP32 ADC is 12-bit (0..4095). Soil pot: high reading = dry soil.
const int DRY_THRESHOLD = 2800;
const int WET_FLOOR     = 1200;

// Pins (ESP32 raw GPIO numbers)
const uint8_t SOIL_PIN        = 34;   // analog in
const uint8_t TANK_LEVEL_PIN  = 14;   // digital in w/ pullup: LOW=water present, HIGH=empty
const uint8_t PUMP_PIN        = 13;   // digital out -> relay IN

const bool PUMP_ACTIVE_LOW = true;

// Sim-tuned timings
const unsigned long PUMP_BURST_MS      = 3UL  * 1000UL;
const unsigned long SOAK_WAIT_MS       = 8UL  * 1000UL;
const unsigned long PUMP_COOLDOWN_MS   = 30UL * 1000UL;
const unsigned long SAMPLE_INTERVAL_MS = 5UL  * 1000UL;
...
int readMoistureRaw() {
  long sum = 0;
  for (int i = 0; i < 8; i++) { sum += analogRead(SOIL_PIN); delay(10); }
  return (int)(sum / 8);
}
...
bool isTankEmpty() {
  // INPUT_PULLUP: LOW means probe is shorted by water = tank has water.
  // HIGH means probe is open circuit = no water bridging it = tank empty.
...
void setup() {
  ...
  Serial.println("Plant agent online (ESP32 SIM).");
  Serial.println("Soil knob = moisture (right=dry). Tank switch = water level (off=empty).");
}
...
void loop() {
  ...
  switch (state) {
    ...
    case IDLE: {
      ...
      if (isDry && tankDry) {
        // Refuse to run a dry pump. Throttle the SOS so we don't spam.
        ...
        sendTelegram("Soil still dry but daily pump quota reached. Possible blockage? (" +
                     String(pct) + "%)");
        lastPumpAt = now;
      }
      break;
    ...
    case SOAKING: {
      if ((now - soakStartedAt) >= SOAK_WAIT_MS) {
        ...
        if (raw >= DRY_THRESHOLD) {
          String msg = "Tried to water but soil is still dry (" + String(pct) + "%).";
